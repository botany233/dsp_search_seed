...
string UTF8ToGBK(const string& utf8Data) {
	int wideCharCount = MultiByteToWideChar(CP_UTF8,0,utf8Data.c_str(),static_cast<int>(utf8Data.length()),nullptr,0);
	vector<wchar_t> wideBuffer(wideCharCount);
	int result = MultiByteToWideChar(CP_UTF8,0,utf8Data.c_str(),static_cast<int>(utf8Data.length()),wideBuffer.data(),wideCharCount);
	int gbkByteCount = WideCharToMultiByte(936,0,wideBuffer.data(),wideCharCount,nullptr,0,nullptr,nullptr);
	string gbkstr(gbkByteCount,0); // Initialize string with the correct size
	result = WideCharToMultiByte(936,0,wideBuffer.data(),wideCharCount,gbkstr.data(),gbkByteCount,nullptr,nullptr);
	return gbkstr;
}
...
namespace pyjson
{
    ...
    inline nl::json to_json(const py::handle& obj, std::set<const PyObject*>& refs)
    {
        ...
        {
            return UTF8ToGBK(obj.cast<std::string>());
        }
