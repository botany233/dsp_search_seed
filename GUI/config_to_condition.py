...
def config_to_galaxy_condition(galaxy_cfg: GalaxyCondition) -> dict:
    galaxy_condition = {
        "valid_state": galaxy_cfg.valid_state,
        "bonds": [config_to_bond_condition(bond_cfg) for bond_cfg in galaxy_cfg.bond_condition],
        "stars": [config_to_star_condition(star_cfg) for star_cfg in galaxy_cfg.star_condition],
        "planets": [config_to_planet_condition(planet_cfg) for planet_cfg in galaxy_cfg.planet_condition],
    }
...
def config_to_bond_condition(bond_cfg: BondCondition) -> dict:
    bond_condition = {
        "valid_state": bond_cfg.valid_state,
        "con1_is_planet": bond_cfg.con1_is_planet,
        "con2_is_planet": bond_cfg.con2_is_planet,
        "con1": config_to_planet_condition(bond_cfg.con1_planet) if bond_cfg.con1_is_planet else config_to_star_condition(bond_cfg.con1_star),
        "con2": config_to_planet_condition(bond_cfg.con2_planet) if bond_cfg.con2_is_planet else config_to_star_condition(bond_cfg.con2_star),
    }
...
def config_to_star_condition(star_cfg: StarCondition) -> dict:
    star_condition = {
        "valid_state": star_cfg.valid_state,
        "planets": [config_to_planet_condition(planet_cfg) for planet_cfg in star_cfg.planet_condition],
    }
...
def config_to_planet_condition(planet_cfg: PlanetCondition) -> dict:
    planet_condition = {
        "valid_state": planet_cfg.valid_state,
        "moons": [config_to_planet_condition(moon_cfg) for moon_cfg in planet_cfg.moon_conditions],
    }
